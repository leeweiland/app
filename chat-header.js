...
(function () {
  ...
  async function build() {
    ...
    if (isAdmin) {
      // visW/visH are the VISIBLE (post-rotation, for the "↻" modes)
      // footprint — what actually shows on screen, used to center the
      // frame. That's different from .pra-root's own CSS width/height for
      // the two rotated modes (which stays the portrait size — see the
      // CSS comment above); centering has to be computed from the rotated
      // shape, not the pre-rotation box, or it'd sit off-center.
      const DEVICE_MODES = [
        { key: 'desktop', label: 'Desktop', cls: null },
        { key: 'tablet',  label: 'Tablet',  cls: 'pra-device-tablet', w: 768, h: 1024 },
        { key: 'mobile',  label: 'Mobile',  cls: 'pra-device-mobile', w: 390, h: 844 },
      ];
      const STORE_KEY = 'pra_device_preview';
      deviceStrip = document.createElement('div');
      deviceStrip.id = 'pra-device-strip';
      const lbl = document.createElement('span');
      lbl.className = 'pds-label';
      lbl.textContent = 'PREVIEW';
      deviceStrip.appendChild(lbl);

      let currentMode = DEVICE_MODES[0];

      // Caps the simulated device's own width/height (overriding the fixed
      // 768x1024 / 390x844 from CSS) to whatever room is actually available
      // below the strip, so the frame — and anything fixed to its bottom
      // edge, like the bottom nav bar — never renders somewhere the real
      // window can't show it.
      //
      // A transform: scale() was tried first (matching real devtools' device
      // toolbar) but had to be reverted: reflow() below sizes the header bar
      // and its spacer using getBoundingClientRect(), which reports the
      // POST-transform (visually shrunk) size, while the `top`/`height`
      // styles reflow() writes are interpreted in the PRE-transform local
      // coordinate space — mixing the two put the bars at the wrong spot on
      // every page, not just the ones taller than the frame. Resizing the
      // frame's actual box instead keeps every measurement in one coordinate
      // space, at the cost of the simulated device no longer being an exact
      // 768x1024/390x844 once it's been capped down.
      fitDeviceFrame = () => {
        if (currentMode.key === 'desktop' || !currentMode.w) { root.style.width = ''; root.style.height = ''; return; }
        const stripH = deviceStrip.getBoundingClientRect().height;
        const availW = window.innerWidth - 16;
        const availH = window.innerHeight - stripH - 16;
        root.style.width = Math.min(currentMode.w, availW) + 'px';
        root.style.height = Math.min(currentMode.h, availH) + 'px';
      };

      function applyMode(key) {
        DEVICE_MODES.forEach(m => { if (m.cls) root.classList.remove(m.cls); });
        // Falls back to 'desktop' for a key from an older saved preference
        // this version no longer knows (e.g. a dropped rotated mode) rather
        // than leaving .pra-device-frame positioning applied with no size
        // class to go with it.
        const mode = DEVICE_MODES.find(m => m.key === key) || DEVICE_MODES[0];
        currentMode = mode;
        root.classList.toggle('pra-device-frame', mode.key !== 'desktop');
        if (mode.cls) root.classList.add(mode.cls);
        deviceStrip.querySelectorAll('button').forEach(b => b.classList.toggle('active', b.dataset.key === mode.key));
        try { localStorage.setItem(STORE_KEY, mode.key); } catch {}
        fitDeviceFrame();
        requestReflow();
      }

      DEVICE_MODES.forEach(m => {
        const b = document.createElement('button');
        b.type = 'button';
        b.textContent = m.label;
        b.dataset.key = m.key;
        b.addEventListener('click', () => applyMode(m.key));
        deviceStrip.appendChild(b);
      });

      // Mounted on <html>, not <body> — on pages that fall back to
      // document.body as `root` (personality-quiz/*), body itself gets
      // .pra-device-frame's transform, which makes it the containing block
      // for ITS OWN fixed-position children. A strip inserted as a body
      // child would get trapped inside that shrunk/repositioned coordinate
      // space instead of sitting above it at the real page top — exactly
      // the "header overlapping the strip" bug this caused. <html> is never
      // the element `root` resolves to, so this is safe in both cases
      // (.pra-root pages and the body-fallback ones alike).
      document.documentElement.insertBefore(deviceStrip, document.body);
      let saved = 'desktop';
      try { saved = localStorage.getItem(STORE_KEY) || 'desktop'; } catch {}
      applyMode(saved);
      new ResizeObserver(() => {
        document.documentElement.style.setProperty('--pra-device-strip-h', deviceStrip.getBoundingClientRect().height + 'px');
        requestReflow();
      }).observe(deviceStrip);
    }
